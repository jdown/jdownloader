...
public class Browser {
	...
	public class BrowserException extends IOException {
		...
		public BrowserException(final String message,
				final URLConnectionAdapter con, final Exception e) {
			this(message, con);
			this.e = e;
		}
	...
	public static String correctURL(String url) {
		...
		if (begin > 0
				&& (!url.substring(begin + 3).contains("//") || !url
						.contains("./"))) {
			return url;
		}
		String ret = url;
	 ...
	 * @throws IOException
	 */
	public static void download(final File file, final URLConnectionAdapter con)
			throws IOException {
		if (file.isFile()) {
			if (!file.delete()) {
		...
		if (con.getHeaderField("Content-Encoding") != null
				&& con.getHeaderField("Content-Encoding").equalsIgnoreCase(
						"gzip")) {
			input = new BufferedInputStream(new GZIPInputStream(con
					.getInputStream()));
		} else {
	...
	public static synchronized void setRequestIntervalLimitGlobal(
			final String host, final int i) {
	...
	private static synchronized void waitForPageAccess(final Browser browser,
		...
		try {
			...
			final long dif = Math.max(localLimit
					- (System.currentTimeMillis() - localLastRequest),
					globalLimit
							- (System.currentTimeMillis() - globalLastRequest));
	 ...
	 */
	private void checkContentLengthLimit(final Request request)
			throws BrowserException {
		if (request == null
				|| request.getHttpConnection() == null
				|| request.getHttpConnection().getHeaderField("Content-Length") == null) {
			return;
		} else if (Long.parseLong(request.getHttpConnection().getHeaderField(
				"Content-Length")) > limit) {
			if (getLogger() != null) {
				getLogger().severe(request.printHeaders());
			}
			throw new BrowserException("Content-length too big", request
					.getHttpConnection());
		}
	}
	...
	public Request createFormRequest(final Form form) throws Exception {
		...
		try {
			// find base in source
			final String sourceBase = this.getRegex("<base.*?href=\"(.+?)\"")
					.getMatch(0).trim();
			// check if valid url
			new URL(sourceBase);
			base = sourceBase;
		...
		case POST:
			if (form.getEncoding() == null
					|| !form.getEncoding().toLowerCase().endsWith("form-data")) {
				return this.createPostRequest(action, form
						.getRequestVariables(), form.getEncoding());
			} else {
				...
				for (int i = 0; i < size; i++) {
					...
					if (entry.getValue() == null) {
						// continue;
							...
							&& entry.getType().equalsIgnoreCase("image")) {
						request.addFormData(new FormData(entry.getKey() + ".x",
								entry.getProperty("x",
										(int) (Math.random() * 100) + "")));
						request.addFormData(new FormData(entry.getKey() + ".y",
								entry.getProperty("y",
										(int) (Math.random() * 100) + "")));
					} else if (entry.getType() != null
							&& entry.getType().equalsIgnoreCase("file")) {
						request.addFormData(new FormData(entry.getKey(), entry
								.getFileToPost().getName(), entry
								.getFileToPost()));
	 ...
	 *             Signals that an I/O exception has occurred.
	 */
	public Request createGetRequest(String string, final Request oldRequest)
			throws IOException {
	 ...
	 *             Signals that an I/O exception has occurred.
	 */
	public Request createGetRequestRedirectedRequest(final Request oldRequest)
			throws IOException {
	...
	private Request createPostRequest(String url,
			final ArrayList<RequestVariable> post, final String encoding)
			throws IOException {
		url = getURL(url);
		boolean sendref = true;
		if (currentURL == null) {
	 ...
	 */
	public Request createPostRequest(final String url,
			final LinkedHashMap<String, String> post) throws IOException {
		return this.createPostRequest(url,
				PostRequest.variableMaptoArray(post), null);
	}

	/**
	 * Creates a postrequest based on a querystring
	 */
	public Request createPostRequest(final String url, final String post)
			throws MalformedURLException, IOException {
		return createPostRequest(url, Request.parseQuery(post));
	}

	public Request createPostRequestfromRedirectedRequest(
			final Request oldrequest, final String postdata) throws IOException {
	 ...
	 * @throws IOException
	 */
	public void downloadConnection(final File file, URLConnectionAdapter con)
			throws IOException {
		if (con == null) {
	...
	public String followConnection() throws IOException {
		...
		try {
			...
			throw new BrowserException(e.getMessage(), request
					.getHttpConnection(), e).closeConnection();
		}
		if (isVerbose()) {
			if (getLogger() != null) {
				getLogger().finest("\r\n" + request + "\r\n");
	...
	private String getBase(final String string) {
		...
		return port != 80 && port > 0 ? "http://" + host + ":" + port + path
				+ "/" : "http://" + host + path + "/";
	}
	...
	public String getBaseURL() {
		...
		// return base + "/";
		// }
		return base.matches("http://.*/.*") ? base.substring(0, base
				.lastIndexOf("/"))
				+ "/" : base + "/";
	}
	...
	public void getDownload(final File file, final String urlString)
			throws IOException {
		final URLConnectionAdapter con = openGetConnection(URLDecoder.decode(
				urlString, "UTF-8"));
		Browser.download(file, con);
	}
	...
	public Form getFormbyProperty(final String property, final String name) {
		for (final Form form : getForms()) {
			if (form.getStringProperty(property) != null
					&& form.getStringProperty(property).equalsIgnoreCase(name)) {
				return form;
			}
		}
		return null;
			...
			if (string.startsWith("/") || string.startsWith("\\")) {
				try {
					...
					if (bUrl.getPort() != 80 && bUrl.getPort() > 0) {
						string = "http://" + new URL(base).getHost() + ":"
								+ bUrl.getPort() + string;
					} else {
		...
		if (con == null) {
			...
			requ = new Request(con) {
				...
				public long postRequest(
						final URLConnectionAdapter httpConnection)
						throws IOException {
					return 0;
				}
	 ...
	 * @throws Exception
	 */
	public URLConnectionAdapter openFormConnection(final Form form)
			throws Exception {
		return openRequestConnection(createFormRequest(form));
	}

	public URLConnectionAdapter openFormConnection(final int i)
			throws Exception {
	 ...
	 * @throws IOException
	 */
	public URLConnectionAdapter openGetConnection(final String string)
			throws IOException {
	 ...
	 */
	public URLConnectionAdapter openPostConnection(final String url,
			final LinkedHashMap<String, String> post) throws IOException {
	 ...
	 */
	public URLConnectionAdapter openRequestConnection(final Request request)
			throws IOException {
		connect(request);
		if (isDebug()) {
			if (getLogger() != null) {
	 ...
	 */
	public String postPage(final String url,
			final LinkedHashMap<String, String> post) throws IOException {
	 ...
	 * loads a new page (POST)
	 */
	public String postPage(final String url, final String post)
			throws IOException {
	 ...
	 * be send as it is
	 */
	public String postPageRaw(final String url, final String post)
			throws IOException {
		final PostRequest request = (PostRequest) this.createPostRequest(url,
				new ArrayList<RequestVariable>(), null);
		request.setCustomCharset(customCharset);
		if (post != null) {
	...
	public void setAuth(final String domain, final String user,
			final String pass) {
	}
	...
	public void setCookie(final String url, final String key, final String value) {
		...
		if (!getCookies().containsKey(host)
				|| (cookies = getCookies().get(host)) == null) {
			cookies = new Cookies();
	...
	public static String getBasicAuthfromURL(String url) {
		if (url == null)
			return null;
		final String basicauth = new Regex(url, "http.*?/([^/]{1}.*?)@")
				.getMatch(0);
