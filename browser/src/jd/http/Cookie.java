...
public class Cookie {
	...
	public boolean equals(final Object obj) {
		...
		} else if (!this.host.equalsIgnoreCase(other.host)) {
			return false;
		}
		if (this.key == null) {
			if (other.key != null) {
	...
	public boolean isExpired() {
		...
		if (this.hostTime == -1) {
			Log.L.severe("Cookie: no HostTime found! ExpireStatus cannot be checked "
					+ this.host + " " + this.key);
			return false;
		} else {
			final long check = System.currentTimeMillis() - this.creationTime
					+ this.hostTime;
