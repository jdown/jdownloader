...
public class Socks5HTTPConnection extends HTTPConnection {
		...
		try {
			...
			if (resp[0] != 1) {
				throw new IOException(
						"Socks5HTTPConnection: invalid Socks5 response");
			}
			if (resp[1] != 0) {
	...
	public void connect() throws IOException {
		...
		if (proxy == null || !proxy.getType().equals(HTTPProxy.TYPE.SOCKS5)) {
		...
		if (httpURL.getProtocol().startsWith("https")) {
			...
			try {
				...
				sslSocket = (SSLSocket) socketFactory.createSocket(
						establishedConnection, httpHost, httpPort, true);
	...
	protected Socket establishConnection() throws IOException {
		try {
			...
			case 8:
				throw new IOException(
						"Socks5HTTPConnection: could not establish connection, status="
								+ resp[1]);
			}
			if (resp[3] == 1) {
				/* ip4v response */
				...
				throw new IOException(
						"Socks5HTTPConnection: unsupported address Type "
								+ resp[3]);
			}
			return socks5socket;
		} catch (final IOException e) {
	...
	protected byte[] readResponse(final int expLength) throws IOException {
		...
		if (index < expLength) {
			throw new IOException("Socks5HTTPConnection: not enough data read");
		}
		return response;
	}
	...
	protected int sayHello() throws IOException {
		try {
			...
			if (resp[1] == 255) {
				throw new IOException(
						"Socks5HTTPConnection: no acceptable authentication method found");
			}
			return resp[1];
		} catch (final IOException e) {
			try {
