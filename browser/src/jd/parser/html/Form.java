...
public class Form {
	...
	public static Form[] getForms(final Object requestInfo) {
		...
		final Matcher formmatcher = pattern.matcher(requestInfo.toString()
				.replaceAll("(?s)<!--.*?-->", ""));
	...
	public String getBestVariable(final String varname) {
		...
		for (final InputField ipf : inputfields) {
			final int dist = EditDistance.getLevenshteinDistance(varname, ipf
					.getKey());
			if (dist < bestDist) {
				best = ipf.getKey();
				bestDist = dist;
	...
	public ArrayList<RequestVariable> getRequestVariables() {
		...
		for (final InputField ipf : inputfields) {
			...
			if (getPreferredSubmit() != null && ipf.getType() != null
					&& ipf.getType().equalsIgnoreCase("submit")
					&& getPreferredSubmit() != ipf) {
				continue;
			}
			if (ipf.getKey() == null) {
				continue;/*
			...
			if (ipf.getType() != null
					&& ipf.getType().equalsIgnoreCase("image")) {
				ret.add(new RequestVariable(ipf.getKey() + ".x", new Random()
						.nextInt(100)
						+ ""));
				ret.add(new RequestVariable(ipf.getKey() + ".y", new Random()
						.nextInt(100)
						+ ""));
			} else {
	...
	private void parse(final String total) {
		...
		//
		// <[\\s]*form(.*?)>(.*?)<[\\s]*/[\\s]*form[\\s]*>|<[\\s]*form(.*?)>(.+)
		final String[][] headerEntries = new Regex(header,
				"[\"' ](\\w+?)[ ]*=[ ]*[\"'](.*?)[\"']").getMatches();
		final String[][] headerEntries2 = new Regex(header,
				"[\"' ](\\w+?)[ ]*=[ ]*([^>^ ^\"^']+)").getMatches();
	...
	private void parseInputFields() {
		...
		final Matcher matcher = Pattern.compile(
				...
				Pattern.CASE_INSENSITIVE).matcher(htmlcode);
		while (matcher.find()) {
	...
	public void remove(final String key) {
		 ...
		 * use iterator here
		 */
		Iterator<InputField> it = inputfields.iterator();
		while (it.hasNext()) {
	...
	public void setPreferredSubmit(int i) {
		...
		for (final InputField ipf : inputfields) {
			if (ipf.getType() != null && ipf.getValue() != null
					&& ipf.getType().equalsIgnoreCase("submit") && i-- <= 0) {
				preferredSubmit = ipf;
				return;
			}
		}
	...
	public void setPreferredSubmit(final String preferredSubmit) {
		...
		for (final InputField ipf : inputfields) {
			if (ipf.getType() != null && ipf.getValue() != null
					&& ipf.getType().equalsIgnoreCase("submit")
					&& ipf.getValue().equalsIgnoreCase(preferredSubmit)) {
				this.preferredSubmit = ipf;
				return;
			}
		}
		Log.L
				.warning("No exact match for submit found! Trying to find best match now!");
		for (final InputField ipf : inputfields) {
			if (ipf.getType() != null && ipf.getValue() != null
					&& ipf.getType().equalsIgnoreCase("submit")
					&& ipf.getValue().contains(preferredSubmit)) {
				this.preferredSubmit = ipf;
				return;
			}
		}
		throw new IllegalArgumentException("No such Submitfield: "
				+ preferredSubmit);
